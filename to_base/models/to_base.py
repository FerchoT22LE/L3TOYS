...
class TOBase(models.AbstractModel):
    ...
    def convert_time_to_utc(self, dt, tz_name=None, is_dst=None, naive=False):
        _logger.warning("The method `convert_time_to_utc()` is deprecated. Please use the `convert_local_to_utc()` instead.")
        return self.convert_local_to_utc(dt=dt, force_local_tz_name=tz_name, is_dst=is_dst, naive=naive)
    ...
    def convert_utc_time_to_tz(self, utc_dt, tz_name=None, is_dst=None, naive=False):
        _logger.warning("The method `convert_utc_time_to_tz()` is deprecated. Please use the `convert_utc_to_local()` instead.")
        return self.convert_utc_to_local(utc_dt=utc_dt, force_local_tz_name=tz_name, is_dst=is_dst, naive=naive)
    ...
    def splittor(self, recordset, max_rec_in_batch=None):
        ...
        for idx in range(0, len(recordset), PREFETCH_MAX):
            sub = recordset[idx:idx + PREFETCH_MAX]
            yield sub
